(function(){
  'use strict';
  // ★ 星海工坊 V2.0 dev.30：人物结构化卡（长期事实 / 特性 / 招式 / 物品）闭环；原文归档不再堆入补充设定。
  ...
  function install(doc, win, buttonId){
    ...
    function xywsNpcCollectFacts(){var out={},err='';$$('[data-npc-fact-row]').forEach(function(row){if(err)return;var n=String((row.querySelector('[data-f="name"]')||{}).value||'').trim(),d=String((row.querySelector('[data-f="desc"]')||{}).value||'').trim();if(!n&&!d)return;if(!n){err='长期事实有内容但缺少短标题';return;}if(!xywsNpcSafeKey(n)){err='长期事实标题「'+n+'」不能以 $ / _ 开头，且不能含 . / 反斜杠 / [ ] 等路径字符，请改成普通短标题';return;}if(!d){err='长期事实「'+n+'」缺少内容';return;}if(Object.prototype.hasOwnProperty.call(out,n)){err='长期事实标题重复：「'+n+'」';return;}out[n]=d;});return {ok:!err,msg:err,data:out};}
    function xywsNpcCollectTraits(){var out={},err='';$$('[data-npc-trait-row]').forEach(function(row){if(err)return;var n=String((row.querySelector('[data-f="name"]')||{}).value||'').trim(),school=String((row.querySelector('[data-f="school"]')||{}).value||'').trim(),type=String((row.querySelector('[data-f="type"]')||{}).value||'').trim()||'被动',effect=String((row.querySelector('[data-f="effect"]')||{}).value||'').trim();if(!n&&!school&&!effect)return;if(!n){err='特性有内容但缺少名称';return;}if(!xywsNpcSafeKey(n)){err='特性名称「'+n+'」不能以 $ / _ 开头，且不能含 . / 反斜杠 / [ ] 等路径字符，请改名';return;}if(!effect){err='特性「'+n+'」缺少效果 / 说明';return;}if(Object.prototype.hasOwnProperty.call(out,n)){err='特性名称重复：「'+n+'」';return;}out[n]={系别:school,类型:type,效果:effect};});return {ok:!err,msg:err,data:out};}
    function xywsNpcCollectSkills(){var out={},err='';$$('[data-npc-skill-row]').forEach(function(row){if(err)return;var n=String((row.querySelector('[data-f="name"]')||{}).value||'').trim(),slot=String((row.querySelector('[data-f="slot"]')||{}).value||'').trim(),school=String((row.querySelector('[data-f="school"]')||{}).value||'').trim(),type=String((row.querySelector('[data-f="type"]')||{}).value||'').trim(),raw=String((row.querySelector('[data-f="cost"]')||{}).value||'').trim(),effect=String((row.querySelector('[data-f="effect"]')||{}).value||'').trim();if(!n&&!school&&!type&&!raw&&!effect)return;if(!n){err='招式有内容但缺少名称';return;}if(!xywsNpcSafeKey(n)){err='招式名称「'+n+'」不能以 $ / _ 开头，且不能含 . / 反斜杠 / [ ] 等路径字符，请改名';return;}if(XYWS_SKILL_SLOTS.indexOf(slot)<0){err='招式「'+n+'」档位无效';return;}var cost=Number(raw);if(raw===''||!Number.isFinite(cost)||Math.round(cost)!==cost){err='招式「'+n+'」请填写整数蓝耗';return;}var rg=XYWS_SKILL_COST_RANGES[slot];if(!rg||cost<rg[0]||cost>rg[1]){err='招式「'+n+'」的'+slot+'蓝耗应为 '+rg[0]+'～'+rg[1];return;}if(!effect){err='招式「'+n+'」缺少效果 / 限制';return;}if(Object.prototype.hasOwnProperty.call(out,n)){err='招式名称重复：「'+n+'」';return;}out[n]={档位:slot,系别:school,类型:type,蓝耗:cost,效果:effect};});return {ok:!err,msg:err,data:out};}
    function xywsNpcCollectItems(){var out={},err='';$$('[data-npc-item-row]').forEach(function(row){if(err)return;var n=String((row.querySelector('[data-f="name"]')||{}).value||'').trim(),raw=String((row.querySelector('[data-f="qty"]')||{}).value||'').trim(),cat=String((row.querySelector('[data-f="cat"]')||{}).value||'').trim()||'杂物',desc=String((row.querySelector('[data-f="desc"]')||{}).value||'').trim();if(!n&&!raw&&!desc)return;if(!n){err='物品有内容但缺少名称';return;}if(!xywsNpcSafeKey(n)){err='物品名称「'+n+'」不能以 $ / _ 开头，且不能含 . / 反斜杠 / [ ] 等路径字符，请改名';return;}var qty=raw===''?1:Number(raw);if(!Number.isFinite(qty)||Math.round(qty)!==qty||qty<1){err='物品「'+n+'」数量必须是正整数';return;}if(!desc){err='物品「'+n+'」缺少作用 / 描述';return;}if(Object.prototype.hasOwnProperty.call(out,n)){err='物品名称重复：「'+n+'」';return;}out[n]={数量:qty,类别:cat,描述:desc};});return {ok:!err,msg:err,data:out};}
    function xywsNpcCollectGear(){var out={},err='';$$('[data-npc-gear-row]').forEach(function(row){if(err)return;var n=String((row.querySelector('[data-f="name"]')||{}).value||'').trim(),type=String((row.querySelector('[data-f="type"]')||{}).value||'').trim(),grade=String((row.querySelector('[data-f="grade"]')||{}).value||'寻常').trim(),bond=String((row.querySelector('[data-f="bond"]')||{}).value||'初缔').trim(),effect=String((row.querySelector('[data-f="effect"]')||{}).value||'').trim();if(!n&&!type&&!effect)return;if(!n){err='星器有内容但缺少名称';return;}if(!xywsNpcSafeKey(n)){err='星器名称「'+n+'」不能以 $ / _ 开头，且不能含 . / 反斜杠 / [ ] 等路径字符，请改名';return;}if(!effect){err='星器「'+n+'」缺少效果 / 特征';return;}if(Object.prototype.hasOwnProperty.call(out,n)){err='星器名称重复：「'+n+'」';return;}out[n]={类型:type,品阶:grade,契合:bond,效果:effect};});return {ok:!err,msg:err,data:out};}
    function xywsNpcReadPublish(){
      ...
      rec=xywsSanitizeNpcRecord(rec,kind);var notes=xywsNpcNotes({});var rawText=xywsNpcField('rawText')||xywsExtraField('[data-import-raw-content]')||(legacy&&legacy.rawText)||'';var source=xywsNpcSourceFromRecord(rec,kind,xywsNpcField('debut'),notes,rawText);return {ok:true,source:source,data:{},body:xywsNpcRecordText(source.npc,source.notes)};
    }
    ...
    function xywsNpcPublishHtml(){return ''+
      '<div class="xyws-source"><b>人物 = 结构化长期人物卡。</b> 能明确归属的内容必须进入专用变量；不好归类但会长期成立的内容拆成“长期事实”。原始设定只做档案保留，不自动塞进变量。</div>'+
      '<div class="xyws-field"><label>人物类型</label><select data-npc="kind"><option value="mahou">魔法少女 / 守护者</option><option value="demon">魔人</option><option value="mortal">关键凡人</option><option value="other">其他重要人物</option></select></div>'+
      '<details class="xyws-npcsec" open><summary>基础资料</summary><div class="xyws-pubgrid"><div class="xyws-field"><label>姓名</label><input data-npc="name"></div><div class="xyws-field"><label>身份 / 职业</label><input data-npc="identity"></div><div class="xyws-field"><label>与主角关系</label><input data-npc="relation"></div><div class="xyws-field"><label>等级（1～70，选填）</label><input data-npc="level" inputmode="numeric"></div><div class="xyws-field"><label>能力系别 / 本能</label><input data-npc="school"></div><div class="xyws-field"><label>当前装束 / 武装</label><input data-npc="battlewear"></div></div><div class="xyws-field"><label>外貌 / 体态</label><textarea data-npc="look"></textarea></div><div class="xyws-field"><label>性格</label><textarea data-npc="personality"></textarea></div><div class="xyws-field"><label>言行模式 / 行动偏好</label><textarea data-npc="combatStyle"></textarea></div><div class="xyws-field"><label>背景要点</label><textarea data-npc="background"></textarea></div></details>'+
      '<details class="xyws-npcsec" open><summary>长期事实</summary><div class="xyws-source">这里不要求再选“性格 / 背景 / 爱好”等类别。每条只要有一个短标题，就会进入 <b>人设.长期事实.&lt;标题&gt;</b>，可以单独读取和修改。</div><div data-npc-fact-list></div><button type="button" class="xyws-secondary" data-a="npc-add-fact">＋ 添加长期事实</button></details>'+
      '<details class="xyws-npcsec" open><summary>特性 / 天赋</summary><div class="xyws-source">被动体质、天赋、常驻效果写这里，不再伪装成 0 蓝耗招式。写入 <b>特性.&lt;名称&gt;</b>。</div><div data-npc-trait-list></div><button type="button" class="xyws-secondary" data-a="npc-add-trait">＋ 添加特性</button></details>'+
      '<details class="xyws-npcsec" open><summary>招式 / 技能</summary><div class="xyws-source">正式招式必须给出档位和整数蓝耗；蓝耗仍按现有系统范围校验，不猜档位。</div><div data-npc-skill-list></div><button type="button" class="xyws-secondary" data-a="npc-add-skill">＋ 添加招式</button></details>'+
      '<details class="xyws-npcsec"><summary>物品 / 装备 / 星器</summary><div class="xyws-source">普通武器、眼镜、仿制星器等可作为“物品”并把类别写成“装备”；真正使用星器品阶 / 契合规则的才放“星器”。</div><div data-npc-item-list></div><button type="button" class="xyws-secondary" data-a="npc-add-item">＋ 添加物品 / 装备</button><div class="xyws-source" style="margin-top:12px">真正星器（选填）</div><div data-npc-gear-list></div><button type="button" class="xyws-secondary" data-a="npc-add-gear">＋ 添加星器</button></details>'+
      '<details class="xyws-npcsec"><summary>关系 / 当前剧情</summary><div class="xyws-pubgrid"><div class="xyws-field"><label>代号 / 称号</label><input data-npc="alias"></div><div class="xyws-field"><label>性别</label><input data-npc="gender"></div><div class="xyws-field"><label>种族</label><input data-npc="race"></div><div class="xyws-field"><label>年龄</label><input data-npc="age" inputmode="numeric"></div><div class="xyws-field"><label>阵营</label><input data-npc="camp"></div><div class="xyws-field"><label>当前位置</label><input data-npc="location"></div><div class="xyws-field"><label>侵蚀阶段</label><select data-npc="corruption"><option value="未染">未染</option><option>初蚀</option><option>沉沦</option><option>半堕</option><option>恶堕</option></select></div><div class="xyws-field"><label>登场安排</label><input data-npc="debut" placeholder="仅登记 / 自然安排 / 尽快登场"></div></div><div class="xyws-field"><label>当前处境</label><textarea data-npc="situation"></textarea></div><div class="xyws-field"><label>剧情钩子（只有真正伏笔才填）</label><textarea data-npc="hook"></textarea></div><div class="xyws-field"><label>秘密（只有确实隐瞒的事实才填）</label><textarea data-npc="secret"></textarea></div></details>'+
      '<details class="xyws-npcsec"><summary>原始设定 / 旧版兼容</summary><div class="xyws-field"><label>原始设定 / 作者原文（只存作品档案，不写 MVU）</label><textarea data-npc="rawText" placeholder="可整段保存原人设；不会直接堆到控制台"></textarea></div><div class="xyws-field"><label>旧版补充备注（兼容旧人物）</label><textarea data-npc="freeform" placeholder="仅兼容旧档；新内容优先拆成长期事实 / 特性 / 招式 / 物品"></textarea></div></details>';}
    ...
    function xywsPopulatePublishFromWork(w,mode){
      ...
      if((pubType==='角色'||pubType==='NPC')&&xywsPublishMode==='import-upload'&&w.xywsImportFormat==='sillytavern.worldbook'){var extra=$('[data-pubextra]');if(extra){var raw=typeof w.xywsRawContent==='string'?w.xywsRawContent:String(w.body||'');extra.insertAdjacentHTML('afterbegin','<div class="xyws-field"><label>世界书原文（保留）</label><textarea data-import-raw-content></textarea><div class="xyws-source" style="margin-top:6px">原文完整保留。人物导入会用固定字段名（如“性格：”“外貌：”“行为模式：”“背景：”）做非 AI 提取，写入已有合法变量；识别不到的内容只保留在“原始设定 / 作者原文”档案里，不会塞进补充设定、钩子或秘密，也不会新造变量。</div></div>');var rawBox=$('[data-import-raw-content]');if(rawBox)rawBox.value=raw;}}
